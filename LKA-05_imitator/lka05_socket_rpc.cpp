...
int Socket_RPC_SIGNAL_Object::call_number = 0;
	...
	Socket_RPC_SLOT_Server::Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, LKA05_widg* _app) : QTcpServer(), app(_app)
	{
	...
	Socket_RPC_SLOT_Thread::Socket_RPC_SLOT_Thread(LKA05_widg* _app, int _socketDescriptor) : app(_app), socketDescriptor(_socketDescriptor)
	{}
	...
	Socket_RPC_SLOT_Object::Socket_RPC_SLOT_Object(LKA05_widg* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
...
//				data_map[signal_name]->cond.wakeAll();
	...
	QVariant Socket_RPC_SLOT_Object::new_tm(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
