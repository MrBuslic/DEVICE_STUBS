...
void RPC_ols_SLOT_Object::connect_to_server()
{
	SRPCSignalClass::Instance().toLog(QString("ols slot connecting %1 %2").arg(addr).arg(port));
	_sock = std::shared_ptr<QTcpSocket>(new QTcpSocket);
	_sock->connectToHost(addr,port);
	if (_sock->waitForConnected(3000))
	{
		connected = true;
		SRPCSignalClass::Instance().toLog("ols slot connected");
	}
	else
	{
		connected = false;
		SRPCSignalClass::Instance().toLog(QString("ols slot connection failed %1 %2").arg(_sock->error()).arg(_sock->errorString()));
	}
}

void RPC_ols_SIGNAL_Object::connect_to_server()
{
	SRPCSignalClass::Instance().toLog(QString("ols signal connecting %1 %2").arg(addr).arg(port));
	_sock = std::shared_ptr<QTcpSocket>(new QTcpSocket);
	_sock->connectToHost(addr,port);
	if (_sock->waitForConnected(3000))
	{
		...
		SRPCSignalClass::Instance().toLog("ols signal connected");
	}
	else
	{
		connected = false;
		SRPCSignalClass::Instance().toLog(QString("ols signal connection failed %1 %2").arg(_sock->error()).arg(_sock->errorString()));
	}
}
...
void RPC_ols_SIGNAL_Object::send_connect(QString signal_name, bool _connect)
{
	QByteArray tmp_arr2;
...
void RPC_ols_SLOT_Object::auto_scroll_clicked(int _state)
{
	QVariantList tmp_list;
	tmp_list << QVariant(_state);
	SRPCSignalClass::Instance().toLog(QString("ols dynamic_call auto_scroll_clicked %1").arg(RPCSignalClass::QVariantToString(tmp_list)));
	dynamic_call("auto_scroll_clicked(int)", tmp_list);
	SRPCSignalClass::Instance().toLog("ols dynamic_call finished auto_scroll_clicked");
}
void RPC_ols_SLOT_Object::log_timer_ontimer()
{
	QVariantList tmp_list;
	SRPCSignalClass::Instance().toLog(QString("ols dynamic_call log_timer_ontimer %1").arg(RPCSignalClass::QVariantToString(tmp_list)));
	dynamic_call("log_timer_ontimer()", tmp_list);
	SRPCSignalClass::Instance().toLog("ols dynamic_call finished log_timer_ontimer");
}
