...
void RPC_r732_SIGNAL_Thread::run()
{
	rpc_obj = std::shared_ptr<RPC_r732_SIGNAL_Object>(new RPC_r732_SIGNAL_Object(addr, port));
	rpc_obj->connect_to_server();
	exec();
}
...
void RPC_r732_SIGNAL_Object::read_data()
{
	int tmp_size;
	while (_sock->bytesAvailable())
	{
		...
		while(!tmp_stream.atEnd())
		{
			...
			SRPCSignalClass::Instance().toLog("r732 new signal " + op_name);

			if (op_name == "new_ku(int, int, double, int)")
			{
				int ku_n;
				tmp_stream >> ku_n;
				SRPCSignalClass::Instance().toLog("r732 " + op_name +" call_number "+ QString::number(call_number) + " ku_n = "+RPCSignalClass::QVariantToString(ku_n));
				int length;
				tmp_stream >> length;
				SRPCSignalClass::Instance().toLog("r732 " + op_name +" call_number "+ QString::number(call_number) + " length = "+RPCSignalClass::QVariantToString(length));
				double u;
				tmp_stream >> u;
				SRPCSignalClass::Instance().toLog("r732 " + op_name +" call_number "+ QString::number(call_number) + " u = "+RPCSignalClass::QVariantToString(u));
				int line;
				tmp_stream >> line;
				SRPCSignalClass::Instance().toLog("r732 " + op_name +" call_number "+ QString::number(call_number) + " line = "+RPCSignalClass::QVariantToString(line));
...
void RPC_r732_SLOT_Object::new_message(QVariant dt, int mko, int line, int cwd, QVariantList words, int os)
{
	...
	SRPCSignalClass::Instance().toLog(QString("r732 dynamic_call new_message %1").arg(RPCSignalClass::QVariantToString(tmp_list)));
	dynamic_call("new_message(QVariant, int, int, int, QVariantList, int)", tmp_list);
	SRPCSignalClass::Instance().toLog("r732 dynamic_call finished new_message");
}
void RPC_r732_SLOT_Object::set_new_mbk02_tm(int sadr, int word)
{
	QVariantList tmp_list;
	tmp_list << QVariant(sadr);
	tmp_list << QVariant(word);
	SRPCSignalClass::Instance().toLog(QString("r732 dynamic_call set_new_mbk02_tm %1").arg(RPCSignalClass::QVariantToString(tmp_list)));
