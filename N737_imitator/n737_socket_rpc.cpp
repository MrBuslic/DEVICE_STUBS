...
int n737_Socket_RPC_SLOT_Thread::obj_num = 0;
	...
	{
		setObjectName("n737_Socket_RPC_SIGNAL_Thread");
	}

	void n737_Socket_RPC_SIGNAL_Thread::run()
	{
		rpc_srv = new n737_Socket_RPC_SIGNAL_Server(conn_ip, conn_port);
		rpc_srv->set_app(app);
		SRPCSignalClass::Instance().toLog("n737 signal thread started");
		exec();
	}

	n737_Socket_RPC_SIGNAL_Server::n737_Socket_RPC_SIGNAL_Server(QString _conn_ip, int _conn_port)
	{
	...
	void n737_Socket_RPC_SIGNAL_Server::tcp_slot()
	{
		SRPCSignalClass::Instance().toLog("n737 signal client connected");
		std::shared_ptr<n737_Socket_RPC_SIGNAL_Object> tmp_obj(new n737_Socket_RPC_SIGNAL_Object);
		tmp_obj->set_app(app);
		tmp_obj->set_socket(rpc_server->nextPendingConnection());
		rpc_objects << tmp_obj;
	}

	n737_Socket_RPC_SLOT_Server_Thread::n737_Socket_RPC_SLOT_Server_Thread() : QThread()
	{
		setObjectName("Socket_RPC_SLOT_Server_Thread");
	}

	n737_Socket_RPC_SLOT_Server::n737_Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, N737_widg* _app) : QTcpServer(), app(_app)
	{
	...
	void n737_Socket_RPC_SLOT_Server_Thread::run()
	{
		rpc_srv = new n737_Socket_RPC_SLOT_Server(conn_ip, conn_port, app);
		SRPCSignalClass::Instance().toLog("n737 slot thread started");
		exec();
	}

	void n737_Socket_RPC_SLOT_Thread::run()
	{
		rpc_obj = std::shared_ptr<n737_Socket_RPC_SLOT_Object>(new n737_Socket_RPC_SLOT_Object(app, socketDescriptor));
		exec();
	}
	...
	{
		SRPCSignalClass::Instance().toLog("n737 slot client connected");
		std::shared_ptr<n737_Socket_RPC_SLOT_Thread> tmp_obj(new n737_Socket_RPC_SLOT_Thread(app, socketDescriptor));
		tmp_obj->start();
		rpc_objects << tmp_obj;
	}

	n737_Socket_RPC_SLOT_Object::n737_Socket_RPC_SLOT_Object(N737_widg* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	setObjectName(QString("n737_SLOT_Object_%1").arg(obj_num++));
		operators_map["QuerySlots()"] = &n737_Socket_RPC_SLOT_Object::QuerySlots;
		///////////////////////////////////////////////////////////////////////
		operators_map["new_frame(QString, QVariant)"] = &n737_Socket_RPC_SLOT_Object::new_frame;
	...
	n737_Socket_RPC_SIGNAL_Object::n737_Socket_RPC_SIGNAL_Object() : QObject()
	{
		setObjectName(QString("n737_SIGNAL_Object_%1").arg(obj_num++));
		connect(this, SIGNAL(send_signal(QByteArray*)), this, SLOT(send_signal_slot(QByteArray*)), Qt::BlockingQueuedConnection);
	}

	void n737_Socket_RPC_SIGNAL_Object::send_signal_func(QByteArray* _arr)
	{
		QMutexLocker locker(&signal_mutex);
		emit send_signal(_arr);
	}

	void n737_Socket_RPC_SLOT_Object::sock_error(QAbstractSocket::SocketError _err)
	{
		SRPCSignalClass::Instance().toLog(QString("%1 SLOT SOCK ERROR!!! %2").arg(this->objectName()).arg(_err));
	}

	void n737_Socket_RPC_SIGNAL_Object::set_socket(QTcpSocket* _rpc_socket)
	{
	...
	void n737_Socket_RPC_SIGNAL_Object::sock_error(QAbstractSocket::SocketError _err)
	{
	...
	void n737_Socket_RPC_SIGNAL_Object::set_app(N737_widg* _app)
	{
		app = _app;
	...
	void n737_Socket_RPC_SLOT_Object::read_data()
	{
		//LARGE_INTEGER _freq;
		//LARGE_INTEGER tmp1;
		//LARGE_INTEGER tmp2;
	...
	void n737_Socket_RPC_SIGNAL_Object::send_signal_slot(QByteArray* _arr)
	{
		rpc_socket->write(*_arr);
		rpc_socket->waitForBytesWritten(3000);
	}

	void n737_Socket_RPC_SIGNAL_Object::read_data()
	{
		int tmp_size;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	void n737_Socket_RPC_SIGNAL_Object::new_ku(int ku_n, int length, double u, int line)
	{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	QVariant n737_Socket_RPC_SLOT_Object::QuerySlots(QVariantList& _values)
	{
		QString tmp_string;
	...
	QVariant n737_Socket_RPC_SLOT_Object::new_frame(QVariantList& _values)
	{
		try
		{
