...
void RPC_n737_SLOT_Thread::run()
{
	rpc_obj = std::shared_ptr<RPC_n737_SLOT_Object>(new RPC_n737_SLOT_Object(addr, port));
	rpc_obj->connect_to_server();
	exec();
}

void RPC_n737_SIGNAL_Thread::run()
{
	rpc_obj = std::shared_ptr<RPC_n737_SIGNAL_Object>(new RPC_n737_SIGNAL_Object(addr, port));
	rpc_obj->connect_to_server();
	exec();
}

void RPC_n737_SIGNAL_Object::connectNotify(const QMetaMethod & signal)
{
	if (signal == QMetaMethod::fromSignal(&RPC_n737_SIGNAL_Object::new_ku)) {
		connect_signal("new_ku(int, int, double, int)", true);
	}
}

void RPC_n737_SIGNAL_Object::disconnectNotify(const QMetaMethod & signal)
{
	if (signal == QMetaMethod::fromSignal(&RPC_n737_SIGNAL_Object::new_ku)) {
...
void RPC_n737_SIGNAL_Object::read_data()
{
	int tmp_size;
	while (_sock->bytesAvailable())
	{
		...
		while(!tmp_stream.atEnd())
		{
			...
			SRPCSignalClass::Instance().toLog("n737 new signal " + op_name);

			if (op_name == "new_ku(int, int, double, int)")
			{
				int ku_n;
				tmp_stream >> ku_n;
				SRPCSignalClass::Instance().toLog("n737 " + op_name +" call_number "+ QString::number(call_number) + " ku_n = "+RPCSignalClass::QVariantToString(ku_n));
				int length;
				tmp_stream >> length;
				SRPCSignalClass::Instance().toLog("n737 " + op_name +" call_number "+ QString::number(call_number) + " length = "+RPCSignalClass::QVariantToString(length));
				double u;
				tmp_stream >> u;
				SRPCSignalClass::Instance().toLog("n737 " + op_name +" call_number "+ QString::number(call_number) + " u = "+RPCSignalClass::QVariantToString(u));
				int line;
				tmp_stream >> line;
				SRPCSignalClass::Instance().toLog("n737 " + op_name +" call_number "+ QString::number(call_number) + " line = "+RPCSignalClass::QVariantToString(line));
				...
				SRPCSignalClass::Instance().toLog("n737 signal finished " + op_name +" call_number "+ QString::number(call_number));
			}
		}
	}
}
