...
KPIUServer::KPIUServer(QWidget* parent) : QWidget(parent)
{
	...
	kprd_widget = new KPRD_imitator;
	foi_widget = new RpcFoiWidget;
	bus_grd_layout->addWidget(omnibus_widget, 0, 0);//отладить 
	...
	bus_grd_layout->addWidget(ols_widget, 2, 0);
	bus_grd_layout->addWidget(kprd_widget, 2, 1);
	bus_grd_layout->addWidget(foi_widget, 2, 2);
