...
mn8i_rpc_buffer_class::mn8i_rpc_buffer_class()
{
	for (int i = 0; i < 1; i++)
	{
...
void mn8i_rpc_buffer_class::packet_ready()
{
...
{ 
	Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_mode_cycle(size);
	return 0;   
}  
...
{ 
	Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_sample_period(periodS);
	return 0;  
}  
...
{
	return Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_input_trigger(on);
}
...
ViStatus _VI_FUNC unmn8i_sample_width_q (ViSession mvi, ViUInt16 *widthWord,
	...
	uint frame_width;
	uint width_in_bytes;
	Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_sample_width_q(frame_width, width_in_bytes);
	if (widthWord)
...
ViStatus _VI_FUNC unmn8i_installHandler (ViSession mvi, ViPAttrState handle, ViPAttrState userData)
{
	...
	QObject::connect(static_cast<RPC_mn8i_SIGNAL_Object*>(Smn8i_rpc_buffer_class::Instance().mn8i_signal_thr[mvi-1]->get_obj().get()), &RPC_mn8i_SIGNAL_Object::packet_ready, &Smn8i_rpc_buffer_class::Instance(), &mn8i_rpc_buffer_class::packet_ready);
	return 0;
}
...
ViStatus _VI_FUNC unmn8i_start (ViSession mvi){
	return Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_start();
}
...
return Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_stop();
...
ViStatus _VI_FUNC unmn8i_read_sample (ViSession mvi, ViPReal64 buf, 
	...
	uint _firstTime;
	uint _thisTime;
	Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_read_sample(tmp_buf, _firstTime, _thisTime);
	*buf = tmp_buf;
										...
										ViReal64 *buf, ViUInt32 *realNumSamples)
{
	...
	Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_read_packet(isHot, numSamples, tmp_buffer, _realNumSamples);
...
{	
	uint _num;
	Smn8i_rpc_buffer_class::Instance().mn8i_slot_thr[mvi - 1]->get_mn8i_obj()->unmn8i_num_ready_data(_num);
	*num = _num;
	return 0;
}
