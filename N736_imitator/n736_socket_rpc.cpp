...
int n736_Socket_RPC_SLOT_Thread::obj_num = 0;
	...
	{
		setObjectName("n736_Socket_RPC_SIGNAL_Thread");
	}

	void n736_Socket_RPC_SIGNAL_Thread::run()
	{
		rpc_srv = new n736_Socket_RPC_SIGNAL_Server(conn_ip, conn_port);
		rpc_srv->set_app(app);
		SRPCSignalClass::Instance().toLog("n736 signal thread started");
		exec();
	}

	n736_Socket_RPC_SIGNAL_Server::n736_Socket_RPC_SIGNAL_Server(QString _conn_ip, int _conn_port)
	{
	...
	void n736_Socket_RPC_SIGNAL_Server::tcp_slot()
	{
		SRPCSignalClass::Instance().toLog("n736 signal client connected");
		std::shared_ptr<n736_Socket_RPC_SIGNAL_Object> tmp_obj(new n736_Socket_RPC_SIGNAL_Object);
		tmp_obj->set_app(app);
		tmp_obj->set_socket(rpc_server->nextPendingConnection());
		rpc_objects << tmp_obj;
	}

	n736_Socket_RPC_SLOT_Server_Thread::n736_Socket_RPC_SLOT_Server_Thread() : QThread()
	{
		setObjectName("Socket_RPC_SLOT_Server_Thread");
	}

	n736_Socket_RPC_SLOT_Server::n736_Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, N736_widg* _app) : QTcpServer(), app(_app)
	{
	...
	void n736_Socket_RPC_SLOT_Server_Thread::run()
	{
		rpc_srv = new n736_Socket_RPC_SLOT_Server(conn_ip, conn_port, app);
		SRPCSignalClass::Instance().toLog("n736 slot thread started");
		exec();
	}

	void n736_Socket_RPC_SLOT_Thread::run()
	{
		rpc_obj = std::shared_ptr<n736_Socket_RPC_SLOT_Object>(new n736_Socket_RPC_SLOT_Object(app, socketDescriptor));
		exec();
	}
	...
	{
		SRPCSignalClass::Instance().toLog("n736 slot client connected");
		std::shared_ptr<n736_Socket_RPC_SLOT_Thread> tmp_obj(new n736_Socket_RPC_SLOT_Thread(app, socketDescriptor));
		tmp_obj->start();
		rpc_objects << tmp_obj;
	}

	n736_Socket_RPC_SLOT_Object::n736_Socket_RPC_SLOT_Object(N736_widg* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	...
	n736_Socket_RPC_SIGNAL_Object::n736_Socket_RPC_SIGNAL_Object() : QObject()
	{
		setObjectName(QString("n736_SIGNAL_Object_%1").arg(obj_num++));
		connect(this, SIGNAL(send_signal(QByteArray*)), this, SLOT(send_signal_slot(QByteArray*)), Qt::BlockingQueuedConnection);
	}

	void n736_Socket_RPC_SIGNAL_Object::send_signal_func(QByteArray* _arr)
	{
		QMutexLocker locker(&signal_mutex);
		emit send_signal(_arr);
	}

	void n736_Socket_RPC_SLOT_Object::sock_error(QAbstractSocket::SocketError _err)
	{
		SRPCSignalClass::Instance().toLog(QString("%1 SLOT SOCK ERROR!!! %2").arg(this->objectName()).arg(_err));
	}

	void n736_Socket_RPC_SIGNAL_Object::set_socket(QTcpSocket* _rpc_socket)
	{
	...
	void n736_Socket_RPC_SIGNAL_Object::sock_error(QAbstractSocket::SocketError _err)
	{
	...
	void n736_Socket_RPC_SLOT_Object::read_data()
	{
		//LARGE_INTEGER _freq;
		//LARGE_INTEGER tmp1;
		//LARGE_INTEGER tmp2;
	...
	void n736_Socket_RPC_SIGNAL_Object::send_signal_slot(QByteArray* _arr)
	{
		rpc_socket->write(*_arr);
		rpc_socket->waitForBytesWritten(3000);
	}

	void n736_Socket_RPC_SIGNAL_Object::read_data()
	{
		int tmp_size;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	QVariant n736_Socket_RPC_SLOT_Object::QuerySlots(QVariantList& _values)
	{
		QString tmp_string;
