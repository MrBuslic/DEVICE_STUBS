...
int Socket_RPC_SIGNAL_Object::call_number = 0;
	...
	Socket_RPC_SLOT_Server::Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, PowerWidget* _app) : QTcpServer(), app(_app)
	{
	...
	Socket_RPC_SLOT_Thread::Socket_RPC_SLOT_Thread(PowerWidget* _app, int _socketDescriptor) : app(_app), socketDescriptor(_socketDescriptor)
	{}
	...
	Socket_RPC_SLOT_Object::Socket_RPC_SLOT_Object(PowerWidget* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	...
	void Socket_RPC_SIGNAL_Object::set_app(PowerWidget* _app)
	{
		app = _app;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	void Socket_RPC_SIGNAL_Object::u_on_nk(double volt)
	{
	...
	void Socket_RPC_SIGNAL_Object::u_on_k1(double volt)
	{
	...
	void Socket_RPC_SIGNAL_Object::u_on_k2(double volt)
	{
	...
	QVariant Socket_RPC_SLOT_Object::set_u(QVariantList& _values)
	{
		try
		{
	...
	QVariant Socket_RPC_SLOT_Object::get_i(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
	...
	QVariant Socket_RPC_SLOT_Object::set_i(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
