...
{
	if (signal == QMetaMethod::fromSignal(&RPC_frame_bus_SIGNAL_Object::new_frame)) {
		SRPCSignalClass::Instance().toLog("new_frame connected");
		emit connect_signal("new_frame(QString, QVariant)", true);
	}
}
...
{
	if (signal == QMetaMethod::fromSignal(&RPC_frame_bus_SIGNAL_Object::new_frame)) {
		SRPCSignalClass::Instance().toLog("new_frame disconnected");
		//emit connect_signal("new_frame(QString, QVariant)", false);
	}
}
...
void RPC_frame_bus_SIGNAL_Object::read_data()
{
	...
	while (_sock->bytesAvailable())
	{
		...
		while(!tmp_stream.atEnd())
		{
			...
			if (op_name == "new_frame(QString, QVariant)")
			{
				QString mode;
				tmp_stream >> mode;
...
void RPC_frame_bus_SLOT_Object::make_new_frame(QString mode, QVariant frame_data)
{
	QVariantList tmp_list;
	tmp_list << QVariant(mode);
	tmp_list << QVariant(frame_data);
	SRPCSignalClass::Instance().toLog(QString("frame_bus dynamic_call make_new_frame %1").arg(RPCSignalClass::QVariantToString(tmp_list)));
	dynamic_call("make_new_frame(QString, QVariant)", tmp_list);
	SRPCSignalClass::Instance().toLog("frame_bus dynamic_call finished make_new_frame");
}
