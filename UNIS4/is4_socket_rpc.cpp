...
int is4_Socket_RPC_SLOT_Object::obj_num = 0;
int is4_Socket_RPC_SIGNAL_Object::obj_num = 0;
int is4_Socket_RPC_SIGNAL_Object::call_number = 0;
int is4_Socket_RPC_SLOT_Thread::obj_num = 0;

	is4_Socket_RPC_SIGNAL_Thread::is4_Socket_RPC_SIGNAL_Thread() : QThread()
	{
		setObjectName("is4_Socket_RPC_SIGNAL_Thread");
	}

	void is4_Socket_RPC_SIGNAL_Thread::run()
	{
		rpc_srv = new is4_Socket_RPC_SIGNAL_Server(conn_ip, conn_port);
		rpc_srv->set_app(app);
		SRPCSignalClass::Instance().toLog("is4 signal thread started");
		exec();
	}

	is4_Socket_RPC_SIGNAL_Server::is4_Socket_RPC_SIGNAL_Server(QString _conn_ip, int _conn_port)
	{
	...
	void is4_Socket_RPC_SIGNAL_Server::tcp_slot()
	{
		SRPCSignalClass::Instance().toLog("is4 signal client connected");
		std::shared_ptr<is4_Socket_RPC_SIGNAL_Object> tmp_obj(new is4_Socket_RPC_SIGNAL_Object);
		tmp_obj->set_app(app);
		tmp_obj->set_socket(rpc_server->nextPendingConnection());
		rpc_objects << tmp_obj;
	}

	is4_Socket_RPC_SLOT_Server_Thread::is4_Socket_RPC_SLOT_Server_Thread() : QThread()
	{
		setObjectName("Socket_RPC_SLOT_Server_Thread");
	}

	is4_Socket_RPC_SLOT_Server::is4_Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, RpcIS4Widget* _app) : QTcpServer(), app(_app)
	{
	...
	void is4_Socket_RPC_SLOT_Server_Thread::run()
	{
		rpc_srv = new is4_Socket_RPC_SLOT_Server(conn_ip, conn_port, app);
		SRPCSignalClass::Instance().toLog("is4 slot thread started");
		exec();
	}

	void is4_Socket_RPC_SLOT_Thread::run()
	{
		rpc_obj = std::shared_ptr<is4_Socket_RPC_SLOT_Object>(new is4_Socket_RPC_SLOT_Object(app, socketDescriptor));
		exec();
	}

	is4_Socket_RPC_SLOT_Thread::is4_Socket_RPC_SLOT_Thread(RpcIS4Widget* _app, int _socketDescriptor) : app(_app), socketDescriptor(_socketDescriptor)
	{
	setObjectName(QString("is4_Socket_RPC_SLOT_Thread_%1").arg(obj_num++));
	}

	void is4_Socket_RPC_SLOT_Server::incomingConnection(qintptr socketDescriptor)
	{
		SRPCSignalClass::Instance().toLog("is4 slot client connected");
		std::shared_ptr<is4_Socket_RPC_SLOT_Thread> tmp_obj(new is4_Socket_RPC_SLOT_Thread(app, socketDescriptor));
		tmp_obj->start();
		rpc_objects << tmp_obj;
	}

	is4_Socket_RPC_SLOT_Object::is4_Socket_RPC_SLOT_Object(RpcIS4Widget* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	setObjectName(QString("is4_SLOT_Object_%1").arg(obj_num++));
		...
		operators_map["unis4_RangeMeas(uint)"] = &is4_Socket_RPC_SLOT_Object::unis4_RangeMeas;
		operators_map["unis4_StartCalibr()"] = &is4_Socket_RPC_SLOT_Object::unis4_StartCalibr;
		///////////////////////////////////////////////////////////////////////
		///////////////////////////////////////////////////////////////////////
	...
	is4_Socket_RPC_SIGNAL_Object::is4_Socket_RPC_SIGNAL_Object() : QObject()
	{
		setObjectName(QString("is4_SIGNAL_Object_%1").arg(obj_num++));
		connect(this, SIGNAL(send_signal(QByteArray*)), this, SLOT(send_signal_slot(QByteArray*)), Qt::BlockingQueuedConnection);
	}

	void is4_Socket_RPC_SIGNAL_Object::send_signal_func(QByteArray* _arr)
	{
		QMutexLocker locker(&signal_mutex);
		emit send_signal(_arr);
	}

	void is4_Socket_RPC_SLOT_Object::sock_error(QAbstractSocket::SocketError _err)
	{
		SRPCSignalClass::Instance().toLog(QString("%1 SLOT SOCK ERROR!!! %2").arg(this->objectName()).arg(_err));
	}

	void is4_Socket_RPC_SIGNAL_Object::set_socket(QTcpSocket* _rpc_socket)
	{
	...
	void is4_Socket_RPC_SIGNAL_Object::sock_error(QAbstractSocket::SocketError _err)
	{
	...
	void is4_Socket_RPC_SLOT_Object::read_data()
	{
		//LARGE_INTEGER _freq;
		//LARGE_INTEGER tmp1;
		//LARGE_INTEGER tmp2;
	...
	void is4_Socket_RPC_SIGNAL_Object::send_signal_slot(QByteArray* _arr)
	{
		rpc_socket->write(*_arr);
		rpc_socket->waitForBytesWritten(3000);
	}

	void is4_Socket_RPC_SIGNAL_Object::read_data()
	{
		int tmp_size;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	QVariant is4_Socket_RPC_SLOT_Object::unis4_SetTypeProcess(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
			uint EProcess = _values.at(0).value<uint>();
			int res = app->unis4_SetTypeProcess(EProcess);
	...
	QVariant is4_Socket_RPC_SLOT_Object::unis4_StartACP(QVariantList& _values)
	{
		try
		{
			int res = app->unis4_StartACP();
			SRPCSignalClass::Instance().toLog(QString("%1 return = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(res)));
			return res;
		}
		catch(const std::exception &)
	...
	QVariant is4_Socket_RPC_SLOT_Object::unis4_ResultMeas(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
			double ResMeas = _values.at(0).value<double>();
			uint NumRes = _values.at(1).value<uint>();
			int res = app->unis4_ResultMeas(ResMeas, NumRes);
			_values[0] = ResMeas;
			SRPCSignalClass::Instance().toLog(QString("%1 ResMeas = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values[0])));
			_values[1] = NumRes;
			SRPCSignalClass::Instance().toLog(QString("%1 NumRes = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values[1])));
			with_return = true;
			SRPCSignalClass::Instance().toLog(QString("%1 return = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(res)));
			return res;
		}
	...
	QVariant is4_Socket_RPC_SLOT_Object::unis4_RangeMeas(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
			uint range = _values.at(0).value<uint>();
			int res = app->unis4_RangeMeas(range);
			SRPCSignalClass::Instance().toLog(QString("%1 return = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(res)));
			return res;
		}
		catch(const std::exception &)
	...
	QVariant is4_Socket_RPC_SLOT_Object::unis4_StartCalibr(QVariantList& _values)
	{
		try
		{
			int res = app->unis4_StartCalibr();
			SRPCSignalClass::Instance().toLog(QString("%1 return = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(res)));
			return res;
		}
		catch(const std::exception &)
