#ifndef BOOP_SOCKET_RPC_H
#define BOOP_SOCKET_RPC_H

#include <QObject>
#include <QString>
#include <QTcpServer>
#include <QTcpSocket>
#include "BOOP.h"

#include <QWidget>
#include <QTextEdit>
#include <QWaitCondition>

#include "rpc_loger.h"
#include "socket_rpc.h"
class BOOP_Socket_RPC_SIGNAL_Object : public QObject
{
	Q_OBJECT
public:
	BOOP_Socket_RPC_SIGNAL_Object();
	~BOOP_Socket_RPC_SIGNAL_Object()
	{
	}
	void set_app(BOOP* _app);

	void set_socket(QTcpSocket* _rpc_socket);
signals:
	void send_signal(QByteArray* _arr);
public slots:

	void send_signal_slot(QByteArray* _arr);
	void read_data();
	void sock_error(QAbstractSocket::SocketError _err);
private:
	void send_signal_func(QByteArray* _arr);
	QTcpSocket* rpc_socket;
	QMutex signal_mutex;
	BOOP* app;
	static int obj_num;
	static int call_number;
	QMap<QString, std::shared_ptr<SignalData> > data_map;
};


class BOOP_Socket_RPC_SIGNAL_Server : public QObject
{
	Q_OBJECT
public:
	BOOP_Socket_RPC_SIGNAL_Server(QString _conn_ip, int _conn_port);
	void set_app(BOOP* _app)
	{
		app = _app;
	}
public slots:
	void tcp_slot();
private:
	QTcpServer* rpc_server;
	BOOP* app;
	QList<std::shared_ptr<BOOP_Socket_RPC_SIGNAL_Object> > rpc_objects;
};

class BOOP_Socket_RPC_SIGNAL_Thread : public QThread
{
	Q_OBJECT
public:
	BOOP_Socket_RPC_SIGNAL_Thread();
	void set_app(BOOP* _app)
	{
		app = _app;
	}
	void set_params(QString _conn_ip, int _conn_port)
	{
		conn_ip = _conn_ip;
		conn_port = _conn_port;
	}
	void run();
private:
	BOOP_Socket_RPC_SIGNAL_Server* rpc_srv;
	BOOP* app;
	QString conn_ip;
	int conn_port;
};

class BOOP_Socket_RPC_SLOT_Object : public QObject
{
	Q_OBJECT
public:
	BOOP_Socket_RPC_SLOT_Object(BOOP* _app, int socketDescriptor);
	~BOOP_Socket_RPC_SLOT_Object()
	{
	}
	typedef QVariant (BOOP_Socket_RPC_SLOT_Object::*OPERATOR_EXECUTOR)(QVariantList&);
	typedef QMap<QString, OPERATOR_EXECUTOR> OPERATORS_MAP;
public:
	QVariant QuerySlots(QVariantList& _values);
	QVariant new_message(QVariantList& _values);
	QVariant new_matrix_command(QVariantList& _values);
	QVariant move_boop(QVariantList& _values);
public slots:
	void read_data();
	void sock_error(QAbstractSocket::SocketError _err);
private:
	OPERATORS_MAP operators_map;
	QTcpSocket* rpc_socket;
	BOOP* app;
	bool with_return;
	static int obj_num;
};

class BOOP_Socket_RPC_SLOT_Thread : public QThread
{
	Q_OBJECT
public:
	BOOP_Socket_RPC_SLOT_Thread(BOOP* _app, int _socketDescriptor);
	void run();
	std::shared_ptr<BOOP_Socket_RPC_SLOT_Object> get_obj(){ return rpc_obj; }
	private:
	std::shared_ptr<BOOP_Socket_RPC_SLOT_Object> rpc_obj;
	BOOP* app;
	int socketDescriptor;
};

class BOOP_Socket_RPC_SLOT_Server : public QTcpServer
{
	Q_OBJECT
public:
	BOOP_Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, BOOP* _app);
protected:
	void incomingConnection(qintptr socketDescriptor) Q_DECL_OVERRIDE;
private:
	BOOP* app;
	QList<std::shared_ptr<BOOP_Socket_RPC_SLOT_Thread> > rpc_objects;
};

class BOOP_Socket_RPC_SLOT_Server_Thread : public QThread
{
	Q_OBJECT
public:
	BOOP_Socket_RPC_SLOT_Server_Thread();
	void set_app(BOOP* _app)
	{
		app = _app;
	}
	void set_params(QString _conn_ip, int _conn_port)
	{
		conn_ip = _conn_ip;
		conn_port = _conn_port;
	}
	void run();
private:
	BOOP_Socket_RPC_SLOT_Server* rpc_srv;
	BOOP* app;
	QString conn_ip;
	int conn_port;

};
#endif
