...
int kprd_Socket_RPC_SLOT_Thread::obj_num = 0;
	...
	{
	setObjectName(QString("kprd_Socket_RPC_SLOT_Thread_%1").arg(obj_num++));
	}
...
//				data_map[signal_name]->cond.wakeAll();
	...
	void kprd_Socket_RPC_SIGNAL_Object::sendKPI(QVariantList kpiList)
	{
		...
		descriptor.mutex.lock();
		send_signal_func(&tmp_arr2);
		SRPCSignalClass::Instance().toLog(QString("%1 send_signal sendKPI sended").arg(objectName()));
		descriptor.mutex.lock();
		descriptor.mutex.unlock();
