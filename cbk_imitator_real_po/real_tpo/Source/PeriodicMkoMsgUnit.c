...
/*! \file PeriodicMsgUnit.c
 * \author Гапонов Р.В.
 * \date 2013.06.26
 * \version 1.0.0
 * \brief Модуль содержит функции передачи периодических сообщений по МКО
 */
...
if ((!F4Mode) && (MKO_CMD_DIR(MkoCmd) == MKO_TX))
  ...
  Result = ReadPeriodicMkoMsgData(BaseMkoLine, BaseMkoChan, Cmd, KpaCommand[7], (WORD*) &Data);
  if (Result != REPLY_CMD_DONE) return Result;
  }

// ищем сообщение в списке
if (FindPeriodicMkoMsg(MkoCmd, MkoLine, MkoChan, F4Mode, (WORD*) &Data, &Index)) return REPLY_ALREADY_SET_ERROR;
...
// ищем сообщение в списке
if (!FindPeriodicMkoMsg(MkoCmd, MkoLine, MkoChan, F4Mode, (WORD*) &Data, &Index)) return REPLY_ITEM_NOT_FOUND;
...
for (Index = 0, Found = 0; Index < PeriodicMkoMsgCount; Index++)
  ...
  if ((!F4Mode) && (MKO_CMD_DIR(MkoCmd) == MKO_TX))
    {
    if (PeriodicMkoMsgDataCmp(Data, (WORD*) &PeriodicMkoMsgList[Index].Data, MKO_CMD_SIZE(MkoCmd))) continue;
    }
...
if (((PeriodicMkoMsgLock & 0x02) == 0) && (PeriodicMkoMsgCount)) ProcessPeriodicMkoMsg(); 
...
for (Index = 0; Index < PeriodicMkoMsgCount; Index++)
  ...
  if (Time.time_sec >= PeriodicMkoMsgList[Index].Timer)
    ...
    if (PeriodicMkoMsgList[Index].F4Mode)
      {
      Result = MkoXchgAn(MkoLine, MkoChan, MkoCmd, 1);
      }
    else 
      {
      if (MKO_CMD_DIR(MkoCmd) == MKO_TX) memcpy(&MkoBufA[0][0], &PeriodicMkoMsgList[Index].Data, MKO_CMD_SIZE(MkoCmd) * sizeof(WORD));
