...
//	nomer_vkluchaemogo_komplekta_samo_znachenie->setText(QString("%1").arg(number_komplekta_AOS));
	...
	//AOS_state.cgo_dev_state = 1;
	//AOS_state._words[1].cgo_dev_state = 1;
...
void AOS_widg::update_graphics(int number_komplekta_AOS)
{
	...
	if (aos_modes.kontr == 1){	perviy_komplekt_kontroly->setStyleSheet(" background-color: rgb(142, 198, 156);");}
	else { perviy_komplekt_kontroly->setStyleSheet(" background-color: rgb(204, 204, 204);");	};
	
	if (aos_modes.kontr == 2)	{ vtoroi_komplekt_kontroly->setStyleSheet(" background-color: rgb(142, 198, 156);");}
	else { vtoroi_komplekt_kontroly->setStyleSheet(" background-color: rgb(204, 204, 204);");};
	
	if (aos_modes.kontr == 3)	{ tretiy_komplekt_kontroly->setStyleSheet(" background-color: rgb(142, 198, 156);");}
	...
	if (name_ustroistva == 8)
	{
		...
		{// включить 1-й комплект
		 // признак включения первого комплекта и передача этого признака отправителю
			...
			nomer_vkluchaemogo_komplekta_samo_znachenie->setText(QString("%1").arg(number_komplekta_AOS));
			update_graphics(number_komplekta_AOS);
			if (number_komplekta_AOS != 0) { aos_state.aos_working = 1; };
...
void AOS_widg::new_message(QVariant dt, int mko, int line, int cwd, QVariantList words, int os)
{
	...
	if ((mko == MKO) && (tmp_cwd.adr == adr) && (tmp_cwd.trans_dir == 0))
	{
		...
		if (tmp_cwd.subadr == 3)//Конфигурация АОС
		{
			...
			// 1-e word ------------------------------- start ----------
			aos_state.res_counter = 0; // 1 bit
			//  aos_state.counter       // 15 bit
			// 1-e word ------------------------------- end ------------
			...
			// 3-e word ---------------------------- начало ---------
			aos_state.kontr_dev_working = aos_state.kontr_dev_state;                           // 3 bit
			aos_state.cgo_dev_working = aos_state.cgo_dev_state;                                // 3 bit
			aos_state.gpfm2_dev_working = aos_state.gpfm2_dev_state;                           // 3 bit
			aos_state.gpfm1_dev_working = aos_state.gpfm1_dev_state;                            // 3 bit
			...
			if (((aos_state.gpfm1_dev_state) || (aos_state.gpfm2_dev_state) ||
				(aos_state.cgo_dev_working) || (aos_state.kontr_dev_working)) != 0)	{ aos_state.aos_working = 1; }
			...
			if (aos_state.kontr_dev_state != 0) { aos_state.rabotosposobnost_intef_VMU_ustroistva_kontrily = 1; }
			else { aos_state.rabotosposobnost_intef_VMU_ustroistva_kontrily = 0; };  // 1 bit
			if (aos_state.cgo_dev_state != 0) { aos_state.rabotosposobnost_intef_VMU_ustroistva_CGO = 1; }
			else { aos_state.rabotosposobnost_intef_VMU_ustroistva_CGO = 0; };          // 1 bit
			if (aos_state.gpfm2_dev_state != 0) { aos_state.rabotosposobnost_intef_VMU_PRM_trakta_AFS2 = 1; }
			else { aos_state.rabotosposobnost_intef_VMU_PRM_trakta_AFS2 = 0; };      // 1 bit
			if (aos_state.gpfm1_dev_state != 0) { aos_state.rabotosposobnost_intef_VMU_PRM_trakta_AFS1 = 1; }
			...
			if (aos_state.aos_test_data_ready == 1) { aos_state.ukazatel_gotovnost_dannih_samokontroly_AOS_M_CH = 1; }
