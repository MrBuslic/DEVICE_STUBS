...
int Socket_RPC_SIGNAL_Object::call_number = 0;
	...
	Socket_RPC_SLOT_Server::Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, R733_widg* _app) : QTcpServer(), app(_app)
	{
	...
	Socket_RPC_SLOT_Thread::Socket_RPC_SLOT_Thread(R733_widg* _app, int _socketDescriptor) : app(_app), socketDescriptor(_socketDescriptor)
	{}
	...
	Socket_RPC_SLOT_Object::Socket_RPC_SLOT_Object(R733_widg* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	...
	void Socket_RPC_SIGNAL_Object::set_app(R733_widg* _app)
	{
		app = _app;
