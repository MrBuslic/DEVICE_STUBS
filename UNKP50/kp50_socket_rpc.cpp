...
int kp50_Socket_RPC_SLOT_Object::obj_num = 0;
int kp50_Socket_RPC_SIGNAL_Object::obj_num = 0;
int kp50_Socket_RPC_SIGNAL_Object::call_number = 0;
int kp50_Socket_RPC_SLOT_Thread::obj_num = 0;

	kp50_Socket_RPC_SIGNAL_Thread::kp50_Socket_RPC_SIGNAL_Thread() : QThread()
	{
		setObjectName("kp50_Socket_RPC_SIGNAL_Thread");
	}

	void kp50_Socket_RPC_SIGNAL_Thread::run()
	{
		rpc_srv = new kp50_Socket_RPC_SIGNAL_Server(conn_ip, conn_port);
		rpc_srv->set_app(app);
		SRPCSignalClass::Instance().toLog("kp50 signal thread started");
		exec();
	}

	kp50_Socket_RPC_SIGNAL_Server::kp50_Socket_RPC_SIGNAL_Server(QString _conn_ip, int _conn_port)
	{
	...
	void kp50_Socket_RPC_SIGNAL_Server::tcp_slot()
	{
		SRPCSignalClass::Instance().toLog("kp50 signal client connected");
		std::shared_ptr<kp50_Socket_RPC_SIGNAL_Object> tmp_obj(new kp50_Socket_RPC_SIGNAL_Object);
		tmp_obj->set_app(app);
		tmp_obj->set_socket(rpc_server->nextPendingConnection());
		rpc_objects << tmp_obj;
	}

	kp50_Socket_RPC_SLOT_Server_Thread::kp50_Socket_RPC_SLOT_Server_Thread() : QThread()
	{
		setObjectName("Socket_RPC_SLOT_Server_Thread");
	}

	kp50_Socket_RPC_SLOT_Server::kp50_Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, RpcKP50Widget* _app) : QTcpServer(), app(_app)
	{
	...
	void kp50_Socket_RPC_SLOT_Server_Thread::run()
	{
		rpc_srv = new kp50_Socket_RPC_SLOT_Server(conn_ip, conn_port, app);
		SRPCSignalClass::Instance().toLog("kp50 slot thread started");
		exec();
	}

	void kp50_Socket_RPC_SLOT_Thread::run()
	{
		rpc_obj = std::shared_ptr<kp50_Socket_RPC_SLOT_Object>(new kp50_Socket_RPC_SLOT_Object(app, socketDescriptor));
		exec();
	}

	kp50_Socket_RPC_SLOT_Thread::kp50_Socket_RPC_SLOT_Thread(RpcKP50Widget* _app, int _socketDescriptor) : app(_app), socketDescriptor(_socketDescriptor)
	{
	setObjectName(QString("kp50_Socket_RPC_SLOT_Thread_%1").arg(obj_num++));
	}

	void kp50_Socket_RPC_SLOT_Server::incomingConnection(qintptr socketDescriptor)
	{
		SRPCSignalClass::Instance().toLog("kp50 slot client connected");
		std::shared_ptr<kp50_Socket_RPC_SLOT_Thread> tmp_obj(new kp50_Socket_RPC_SLOT_Thread(app, socketDescriptor));
		tmp_obj->start();
		rpc_objects << tmp_obj;
	}

	kp50_Socket_RPC_SLOT_Object::kp50_Socket_RPC_SLOT_Object(RpcKP50Widget* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	setObjectName(QString("kp50_SLOT_Object_%1").arg(obj_num++));
		...
		operators_map["unkp50_meas_I(int)"] = &kp50_Socket_RPC_SLOT_Object::unkp50_meas_I;
		operators_map["unkp50_meas_Uin(int)"] = &kp50_Socket_RPC_SLOT_Object::unkp50_meas_Uin;
		operators_map["unkp50_meas_Uout(int)"] = &kp50_Socket_RPC_SLOT_Object::unkp50_meas_Uout;
		///////////////////////////////////////////////////////////////////////
		///////////////////////////////////////////////////////////////////////
	...
	kp50_Socket_RPC_SIGNAL_Object::kp50_Socket_RPC_SIGNAL_Object() : QObject()
	{
		setObjectName(QString("kp50_SIGNAL_Object_%1").arg(obj_num++));
		connect(this, SIGNAL(send_signal(QByteArray*)), this, SLOT(send_signal_slot(QByteArray*)), Qt::BlockingQueuedConnection);
	}

	void kp50_Socket_RPC_SIGNAL_Object::send_signal_func(QByteArray* _arr)
	{
		QMutexLocker locker(&signal_mutex);
		emit send_signal(_arr);
	}

	void kp50_Socket_RPC_SLOT_Object::sock_error(QAbstractSocket::SocketError _err)
	{
		SRPCSignalClass::Instance().toLog(QString("%1 SLOT SOCK ERROR!!! %2").arg(this->objectName()).arg(_err));
	}

	void kp50_Socket_RPC_SIGNAL_Object::set_socket(QTcpSocket* _rpc_socket)
	{
	...
	void kp50_Socket_RPC_SIGNAL_Object::sock_error(QAbstractSocket::SocketError _err)
	{
	...
	void kp50_Socket_RPC_SIGNAL_Object::set_app(RpcKP50Widget* _app)
	{
		app = _app;

	}

	void kp50_Socket_RPC_SLOT_Object::read_data()
	{
		//LARGE_INTEGER _freq;
		//LARGE_INTEGER tmp1;
		//LARGE_INTEGER tmp2;
	...
	void kp50_Socket_RPC_SIGNAL_Object::send_signal_slot(QByteArray* _arr)
	{
		rpc_socket->write(*_arr);
		rpc_socket->waitForBytesWritten(3000);
	}

	void kp50_Socket_RPC_SIGNAL_Object::read_data()
	{
		int tmp_size;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	QVariant kp50_Socket_RPC_SLOT_Object::QuerySlots(QVariantList& _values)
	{
		QString tmp_string;
	...
	QVariant kp50_Socket_RPC_SLOT_Object::unkp50_switch_channel(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
	...
	QVariant kp50_Socket_RPC_SLOT_Object::unkp50_channel_state_Q(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
	...
	QVariant kp50_Socket_RPC_SLOT_Object::unkp50_meas_I(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
	...
	QVariant kp50_Socket_RPC_SLOT_Object::unkp50_meas_Uin(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
	...
	QVariant kp50_Socket_RPC_SLOT_Object::unkp50_meas_Uout(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
