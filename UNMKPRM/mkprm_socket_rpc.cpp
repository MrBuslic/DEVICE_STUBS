...
int mkprm_Socket_RPC_SIGNAL_Object::call_number = 0;
	...
	{
		setObjectName("mkprm_Socket_RPC_SIGNAL_Thread");
	}

	void mkprm_Socket_RPC_SIGNAL_Thread::run()
	{
		rpc_srv = new mkprm_Socket_RPC_SIGNAL_Server(conn_ip, conn_port);
		rpc_srv->set_app(app);
		SRPCSignalClass::Instance().toLog("mkprm signal thread started");
		exec();
	}

	mkprm_Socket_RPC_SIGNAL_Server::mkprm_Socket_RPC_SIGNAL_Server(QString _conn_ip, int _conn_port)
	{
	...
	void mkprm_Socket_RPC_SIGNAL_Server::tcp_slot()
	{
		SRPCSignalClass::Instance().toLog("mkprm signal client connected");
		std::shared_ptr<mkprm_Socket_RPC_SIGNAL_Object> tmp_obj(new mkprm_Socket_RPC_SIGNAL_Object);
		tmp_obj->set_app(app);
		tmp_obj->set_socket(rpc_server->nextPendingConnection());
		rpc_objects << tmp_obj;
	}

	mkprm_Socket_RPC_SLOT_Server_Thread::mkprm_Socket_RPC_SLOT_Server_Thread() : QThread()
	{
		setObjectName("Socket_RPC_SLOT_Server_Thread");
	}

	mkprm_Socket_RPC_SLOT_Server::mkprm_Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, RpcMKPRMWidget* _app) : QTcpServer(), app(_app)
	{
	...
	void mkprm_Socket_RPC_SLOT_Server_Thread::run()
	{
		rpc_srv = new mkprm_Socket_RPC_SLOT_Server(conn_ip, conn_port, app);
		SRPCSignalClass::Instance().toLog("mkprm slot thread started");
		exec();
	}

	void mkprm_Socket_RPC_SLOT_Thread::run()
	{
		rpc_obj = std::shared_ptr<mkprm_Socket_RPC_SLOT_Object>(new mkprm_Socket_RPC_SLOT_Object(app, socketDescriptor));
		exec();
	}
	...
	{
		SRPCSignalClass::Instance().toLog("mkprm slot client connected");
		std::shared_ptr<mkprm_Socket_RPC_SLOT_Thread> tmp_obj(new mkprm_Socket_RPC_SLOT_Thread(app, socketDescriptor));
		tmp_obj->start();
		rpc_objects << tmp_obj;
	}

	mkprm_Socket_RPC_SLOT_Object::mkprm_Socket_RPC_SLOT_Object(RpcMKPRMWidget* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	setObjectName(QString("mkprm_SLOT_Object_%1").arg(obj_num++));
		operators_map["QuerySlots()"] = &mkprm_Socket_RPC_SLOT_Object::QuerySlots;
		///////////////////////////////////////////////////////////////////////
		operators_map["unmkprm_get_strings(int, QVariantList&)"] = &mkprm_Socket_RPC_SLOT_Object::unmkprm_get_strings;
		operators_map["unmkprm_start()"] = &mkprm_Socket_RPC_SLOT_Object::unmkprm_start;
		operators_map["unmkprm_stop()"] = &mkprm_Socket_RPC_SLOT_Object::unmkprm_stop;
		operators_map["new_frame(QString, QVariant)"] = &mkprm_Socket_RPC_SLOT_Object::new_frame;
		///////////////////////////////////////////////////////////////////////
		///////////////////////////////////////////////////////////////////////
	...
	mkprm_Socket_RPC_SIGNAL_Object::mkprm_Socket_RPC_SIGNAL_Object() : QObject()
	{
		setObjectName(QString("mkprm_SIGNAL_Object_%1").arg(obj_num++));
		connect(this, SIGNAL(send_signal(QByteArray*)), this, SLOT(send_signal_slot(QByteArray*)), Qt::BlockingQueuedConnection);
	}

	void mkprm_Socket_RPC_SIGNAL_Object::send_signal_func(QByteArray* _arr)
	{
		QMutexLocker locker(&signal_mutex);
		emit send_signal(_arr);
	}

	void mkprm_Socket_RPC_SLOT_Object::sock_error(QAbstractSocket::SocketError _err)
	{
		SRPCSignalClass::Instance().toLog(QString("%1 SLOT SOCK ERROR!!! %2").arg(this->objectName()).arg(_err));
	}

	void mkprm_Socket_RPC_SIGNAL_Object::set_socket(QTcpSocket* _rpc_socket)
	{
	...
	void mkprm_Socket_RPC_SIGNAL_Object::sock_error(QAbstractSocket::SocketError _err)
	{
	...
	void mkprm_Socket_RPC_SIGNAL_Object::set_app(RpcMKPRMWidget* _app)
	{
		app = _app;

	}

	void mkprm_Socket_RPC_SLOT_Object::read_data()
	{
		//LARGE_INTEGER _freq;
		//LARGE_INTEGER tmp1;
		//LARGE_INTEGER tmp2;
	...
	void mkprm_Socket_RPC_SIGNAL_Object::send_signal_slot(QByteArray* _arr)
	{
		rpc_socket->write(*_arr);
		rpc_socket->waitForBytesWritten(3000);
	}

	void mkprm_Socket_RPC_SIGNAL_Object::read_data()
	{
		int tmp_size;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	QVariant mkprm_Socket_RPC_SLOT_Object::QuerySlots(QVariantList& _values)
	{
		QString tmp_string;
	...
	QVariant mkprm_Socket_RPC_SLOT_Object::unmkprm_get_strings(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
			int strings = _values.at(0).value<int>();
			QVariantList string_data = _values.at(1).value<QVariantList>();
			int res = app->unmkprm_get_strings(strings, string_data);
			_values[1] = string_data;
			SRPCSignalClass::Instance().toLog(QString("%1 string_data = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values[1])));
			with_return = true;
	...
	QVariant mkprm_Socket_RPC_SLOT_Object::unmkprm_start(QVariantList& _values)
	{
		try
		{
			app->unmkprm_start();
	...
	QVariant mkprm_Socket_RPC_SLOT_Object::unmkprm_stop(QVariantList& _values)
	{
		try
		{
			app->unmkprm_stop();
	...
	QVariant mkprm_Socket_RPC_SLOT_Object::new_frame(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
			QString mode = _values.at(0).value<QString>();
			QVariant _frame_data = _values.at(1).value<QVariant>();
			app->new_frame(mode, _frame_data);
			return 0;
		}
		catch(const std::exception &)
		{
