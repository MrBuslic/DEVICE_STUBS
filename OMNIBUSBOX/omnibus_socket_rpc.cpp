...
int omnibus_Socket_RPC_SLOT_Object::obj_num = 0;
int omnibus_Socket_RPC_SIGNAL_Object::obj_num = 0;
int omnibus_Socket_RPC_SIGNAL_Object::call_number = 0;
int omnibus_Socket_RPC_SLOT_Thread::obj_num = 0;

	omnibus_Socket_RPC_SIGNAL_Thread::omnibus_Socket_RPC_SIGNAL_Thread() : QThread()
	{
		setObjectName("omnibus_Socket_RPC_SIGNAL_Thread");
	}

	void omnibus_Socket_RPC_SIGNAL_Thread::run()
	{
		rpc_srv = new omnibus_Socket_RPC_SIGNAL_Server(conn_ip, conn_port);
		rpc_srv->set_app(app);
		SRPCSignalClass::Instance().toLog("omnibus signal thread started");
		exec();
	}

	omnibus_Socket_RPC_SIGNAL_Server::omnibus_Socket_RPC_SIGNAL_Server(QString _conn_ip, int _conn_port)
	{
	...
	void omnibus_Socket_RPC_SIGNAL_Server::tcp_slot()
	{
		SRPCSignalClass::Instance().toLog("omnibus signal client connected");
		std::shared_ptr<omnibus_Socket_RPC_SIGNAL_Object> tmp_obj(new omnibus_Socket_RPC_SIGNAL_Object);
		tmp_obj->set_app(app);
		tmp_obj->set_socket(rpc_server->nextPendingConnection());
		rpc_objects << tmp_obj;
	}

	omnibus_Socket_RPC_SLOT_Server_Thread::omnibus_Socket_RPC_SLOT_Server_Thread() : QThread()
	{
		setObjectName("Socket_RPC_SLOT_Server_Thread");
	}

	omnibus_Socket_RPC_SLOT_Server::omnibus_Socket_RPC_SLOT_Server(QString _conn_ip, int _conn_port, RpcOmnibusWidget* _app) : QTcpServer(), app(_app)
	{
	...
	void omnibus_Socket_RPC_SLOT_Server_Thread::run()
	{
		rpc_srv = new omnibus_Socket_RPC_SLOT_Server(conn_ip, conn_port, app);
		SRPCSignalClass::Instance().toLog("omnibus slot thread started");
		exec();
	}

	void omnibus_Socket_RPC_SLOT_Thread::run()
	{
		rpc_obj = std::shared_ptr<omnibus_Socket_RPC_SLOT_Object>(new omnibus_Socket_RPC_SLOT_Object(app, socketDescriptor));
		exec();
	}

	omnibus_Socket_RPC_SLOT_Thread::omnibus_Socket_RPC_SLOT_Thread(RpcOmnibusWidget* _app, int _socketDescriptor) : app(_app), socketDescriptor(_socketDescriptor)
	{
	setObjectName(QString("omnibus_Socket_RPC_SLOT_Thread_%1").arg(obj_num++));
	}

	void omnibus_Socket_RPC_SLOT_Server::incomingConnection(qintptr socketDescriptor)
	{
		SRPCSignalClass::Instance().toLog("omnibus slot client connected");
		std::shared_ptr<omnibus_Socket_RPC_SLOT_Thread> tmp_obj(new omnibus_Socket_RPC_SLOT_Thread(app, socketDescriptor));
		tmp_obj->start();
		rpc_objects << tmp_obj;
	}

	omnibus_Socket_RPC_SLOT_Object::omnibus_Socket_RPC_SLOT_Object(RpcOmnibusWidget* _app, int socketDescriptor) : QObject(), with_return(false), app(_app)
	{
	...
	omnibus_Socket_RPC_SIGNAL_Object::omnibus_Socket_RPC_SIGNAL_Object() : QObject()
	{
	...
	void omnibus_Socket_RPC_SIGNAL_Object::send_signal_func(QByteArray* _arr)
	{
		QMutexLocker locker(&signal_mutex);
		emit send_signal(_arr);
	}

	void omnibus_Socket_RPC_SLOT_Object::sock_error(QAbstractSocket::SocketError _err)
	{
		SRPCSignalClass::Instance().toLog(QString("%1 SLOT SOCK ERROR!!! %2").arg(this->objectName()).arg(_err));
	}

	void omnibus_Socket_RPC_SIGNAL_Object::set_socket(QTcpSocket* _rpc_socket)
	{
	...
	void omnibus_Socket_RPC_SIGNAL_Object::sock_error(QAbstractSocket::SocketError _err)
	{
	...
	void omnibus_Socket_RPC_SLOT_Object::read_data()
	{
		//LARGE_INTEGER _freq;
		//LARGE_INTEGER tmp1;
		//LARGE_INTEGER tmp2;
	...
	void omnibus_Socket_RPC_SIGNAL_Object::send_signal_slot(QByteArray* _arr)
	{
		rpc_socket->write(*_arr);
		rpc_socket->waitForBytesWritten(3000);
	}

	void omnibus_Socket_RPC_SIGNAL_Object::read_data()
	{
		int tmp_size;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	void omnibus_Socket_RPC_SIGNAL_Object::new_message(QVariant dt, int mko, int line, int cwd, QVariantList words, int os)
	{
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	/////////////////////////////////////////////////////////////////////////////////////////////////////////
	QVariant omnibus_Socket_RPC_SLOT_Object::QuerySlots(QVariantList& _values)
	{
		QString tmp_string;
	...
	QVariant omnibus_Socket_RPC_SLOT_Object::set_new_data(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
	...
	QVariant omnibus_Socket_RPC_SLOT_Object::send_msg(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
