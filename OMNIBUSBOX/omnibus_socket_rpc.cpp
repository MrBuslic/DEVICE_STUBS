...
int Socket_RPC_SIGNAL_Object::call_number = 0;
	...
	setObjectName(QString("omnibus_SLOT_Object_%1").arg(obj_num++));
		...
		operators_map["switch_ab_os(int, int, int)"] = &Socket_RPC_SLOT_Object::switch_ab_os;
		...
		operators_map["unomnibus_map_channels_setup(int, short)"] = &Socket_RPC_SLOT_Object::unomnibus_map_channels_setup;
...
//				data_map[signal_name]->cond.wakeAll();
	...
	QVariant Socket_RPC_SLOT_Object::switch_ab_os(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
	...
	QVariant Socket_RPC_SLOT_Object::unomnibus_map_channels_setup(QVariantList& _values)
	{
		try
		{
			SRPCSignalClass::Instance().toLog(QString("%1 _values = %2").arg(objectName()).arg(RPCSignalClass::QVariantToString(_values)));
